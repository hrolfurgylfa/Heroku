...
def Blog():
    ...
    for notandi in notendur:
        if notandi["USERNAME"] == username:
            return template("Blog/forsida.tpl", f_d = flokkad_dagsetning, f_l = flokkad_like, u = username)
...
def Blog_oll_blog():
    ...
    sina_notanda = request.query.u or False
    db = database("tsuts.tskoli.is", "2109013290", "mypassword", "2109013290_blog")
    notandanafn = tekkaInnskraningu()

    if flokkad is False and sina_notanda is False:
        flokkad_dagsetning = db.executeSQL("SELECT ID, DAGSETNING, LIKES, TITILL, TEXTI, POST_OWNER, NAFN FROM POSTS JOIN USERS ON POSTS.POST_OWNER = USERS.USERNAME ORDER BY DAGSETNING DESC")
        flokkad_like = db.executeSQL("SELECT ID, DAGSETNING, LIKES, TITILL, TEXTI, POST_OWNER, NAFN FROM POSTS JOIN USERS ON POSTS.POST_OWNER = USERS.USERNAME ORDER BY LIKES DESC")
        
        return template("Blog/blog_oflokkad.tpl", f_l = flokkad_like, f_d = flokkad_dagsetning, n = notandanafn)
    
    elif flokkad == "mest_likad":
        flokkad_like = db.executeSQL("SELECT ID, DAGSETNING, LIKES, TITILL, TEXTI, POST_OWNER, NAFN FROM POSTS JOIN USERS ON POSTS.POST_OWNER = USERS.USERNAME ORDER BY LIKES DESC")
        return template("Blog/blog_flokkad.tpl", listi = flokkad_like, t = "Vinsælustu blog", n = notandanafn)

    elif flokkad == "nyjast":
        flokkad_dagsetning = db.executeSQL("SELECT ID, DAGSETNING, LIKES, TITILL, TEXTI, POST_OWNER, NAFN FROM POSTS JOIN USERS ON POSTS.POST_OWNER = USERS.USERNAME ORDER BY DAGSETNING DESC")
        return template("Blog/blog_flokkad.tpl", listi = flokkad_dagsetning, t = "Nýjustu blog", n = notandanafn)

    elif sina_notanda is not False:
        flokkad_notanda = db.executeSQL("SELECT ID, DAGSETNING, LIKES, TITILL, TEXTI, POST_OWNER, NAFN FROM POSTS JOIN USERS ON POSTS.POST_OWNER = USERS.USERNAME WHERE POST_OWNER = '"+str(sina_notanda)+"' ORDER BY DAGSETNING DESC")
        try:
            nafn_notanda_i_leit = flokkad_notanda[0]["NAFN"]
        except IndexError:
...
@route("/blog/innskra_notenda", method="POST")
def Blog_innskra_notenda():
    db = database("tsuts.tskoli.is", "2109013290", "mypassword", "2109013290_blog")
    notendur = db.executeSQL("SELECT * FROM USERS")
    notendanafn = request.forms.notendanafn
    lykilord = request.forms.adgangsord

    for notandi in notendur:
        if notandi["USERNAME"] == notendanafn:
            if notandi["ADGANGSORD"] == lykilord:
                response.set_cookie("innskraning_hrolfurgylfa_heroku_blog", notendanafn, secret="")
                return redirect("/blog")
    
    return template("Blog/error.tpl", t = "Rangt notendanafn eða lykilorð", l = "/blog/innskraning")

@route("/blog/utskra")
def Blog_utskra():
    response.delete_cookie("innskraning_hrolfurgylfa_heroku_blog", secret='')
    return redirect("/blog")

@route("/blog/bua_til_adgang")
def Blog_bua_til_adgang():
    return template("Blog/bua_til_adgang.tpl")
